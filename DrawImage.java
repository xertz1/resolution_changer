...
public class DrawImage extends JComponent {
	...
	public DrawImage(String imagefile, int resolution) {
		...
		try {
			...
			if (size <= SIZE_CANVAS)
				size = SIZE_CANVAS;
			else if (size <= 2 * SIZE_CANVAS)
				size = 2 * SIZE_CANVAS;
			else if (size <= 4 * SIZE_CANVAS)
				size = 4 * SIZE_CANVAS;
			else {
			...
			ListNode<QTreeNode> head = imageTree.getPixels(imageTree.getRoot(), resolution);
			storePixels(head);
		} catch (IOException e) {
	...
	public DrawImage(int resolution, int x, int y, boolean mark) {
		...
		System.out.println(
				"Number of pixels with the same color as pixel (" + x + "," + y + "): " + headColor.getCount());
	}
